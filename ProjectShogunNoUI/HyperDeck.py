...
class HyperDeck:
    ...
    def __init__(self, host, port=None, window=None, loop=None):
        self.host = host
    ...
    async def record(self, name = None):
        if name is None:
            print('invalid name')
        else:
    ...
    async def delete(self, clip):
        command = 'clips remove: clip id: {}'.format(clip)
        response = await self._send_command(command)
        return response and not response['error']
    ...
    async def update_status(self):
        ...
        disp = str(response['lines'][1]) + ' ' + str(self.host)
        self.code = disp
